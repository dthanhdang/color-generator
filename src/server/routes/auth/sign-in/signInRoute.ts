...
import { signInHandler, signInJSONSchema } from "./signInHandler.js"

export const signInRoute = buildHono().post(
  "/",
  vValidator("json", signInJSONSchema),
  async (c) => {
    return signInHandler({ ...dbProps(c), ...c.req.valid("json") })
      .andThen((user) =>
